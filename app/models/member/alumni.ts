...
export class $MemberAlumni<Mode extends ModelMode = 'DEFAULT'> implements ThisModelImpl<Mode> {
  ...
  public static with(client: PrismaClient) {
    return (builder: ModelBuilderType) => ({
      __build: (args: RawData) => new $MemberAlumni(client, args, builder),
      ...
      from: (memberId: MemberId) => safeTry(async function* () {
        ...
        const rawData = yield * await databaseWrapBridgeResult(
          client.memberAlumni.findUniqueOrThrow({ where: { memberId } }),
        ).map(separateRD.default);

        return ok(new $MemberAlumni(client, rawData, builder));
      }).mapErr(dbError('from')),
      ...
      fromWithResolved: (memberId: MemberId) => safeTry(async function* () {
        ...
        const rawData = yield * await databaseWrapBridgeResult(
          client.memberAlumni.findUniqueOrThrow({ where: { memberId }, include: includeKeys2select(includeKeys) }),
        ).map(separateRD.withResolved);

        return ok(new $MemberAlumni<'WITH_RESOLVED'>(client, rawData, builder));
      ...
      findMany: (args) => safeTry(async function* () {
        ...
        return ok(rawData.map((data) => new $MemberAlumni(client, data, builder)));
  ...
  public resolveRelation(): ModelResolver<Mode, ThisModel> {
    return matchWithDefault(
      this.__rawResolved,
      () => $MemberAlumni.with(this.client)(this.builder).fromWithResolved(this.data.memberId),
    );
  }
  ...
  public update(data: Partial<Schema>): DatabaseResult<ThisModel> {
    ...
    return databaseWrapBridgeResult(
      this.client.memberAlumni.update({ data: __raw, where: { memberId: this.data.memberId } }),
    )
      .mapErr(dbError('update'))
      .map(separateRD.default)
      .map((r) => new $MemberAlumni(this.client, r, this.builder));
  }
